set -e

function make_necessary_dirs {
    mkdir -p data
    mkdir -p bgm
    cd data
    mkdir -p ascii
    mkdir -p demo
    mkdir -p eff00
    mkdir -p end
    mkdir -p etama
    mkdir -p face
    mkdir -p front
    mkdir -p image
    mkdir -p player00
    mkdir -p result
    mkdir -p stg1bg
    mkdir -p stgenm
    mkdir -p title
    mkdir -p wav
}

merge_from_list() {
    local infile="$1"

    while IFS= read -r line; do
        [[ -z "$line" ]] && continue
        [[ "$line" =~ ^color:\ (.*) ]] && color="${BASH_REMATCH[1]}"
        [[ "$line" =~ ^alpha:\ (.*) ]] && alpha="${BASH_REMATCH[1]}"

        if [[ -n "$color" && -n "$alpha" ]]; then
            out="${color%.png}_merged.png"

            magick "$color" -alpha off "$alpha" -compose CopyOpacity -composite "$out"

            mv "$out" "$color"

            unset color alpha
        fi
    done < "$infile"
}

process_anm_folder() {
    local dir="$1"

    for anm in "$dir"/*.anm; do
        echo "Processing $anm..."
        tmp=$(mktemp)
        $INVOCATION_LOCATION/scripts/get_anm_texture_names "$anm" > "$tmp"
        merge_from_list "$tmp"
        rm "$tmp"
    done
}

INVOCATION_LOCATION=$(pwd)
GAME_LOCATION=$1

if [ -z "$GAME_LOCATION" ]; then
    echo "Usage: $0 <game_location>"
    exit 1
fi

if [ ! -x "$INVOCATION_LOCATION/scripts/get_anm_texture_names" ]; then
    echo "Error: get_anm_texture_names script not found or not executable! Attempting to compile it now.."

    CC=clang

    if [ -z "$CC" ]; then
        CC=gcc
    fi

    cd "$INVOCATION_LOCATION/scripts"
    $CC get_anm_texture_names.cpp -o get_anm_texture_names
    cd "$INVOCATION_LOCATION"

    if [ ! -x "$INVOCATION_LOCATION/scripts/get_anm_texture_names" ]; then
        echo "Error: Failed to compile get_anm_texture_names script!"
        exit 1
    else
        echo "Successfully compiled get_anm_texture_names script! Continuing..."
    fi
fi

cd $GAME_LOCATION
make_necessary_dirs

for file in $GAME_LOCATION*.DAT; do
    printf "Processing $file..."
    mkdir -p thdat_output
    thdat -x 6 "$file" -C $GAME_LOCATION/data/thdat_output > /dev/null
    printf " Done!\n"
done;

echo "Moving files..."

cd $GAME_LOCATION/data
mv thdat_output/ascii*.png ./ascii
mv thdat_output/demo00.rpy ./demo
mv thdat_output/eff*.png ./eff00
mv thdat_output/end*.jpg ./end
mv thdat_output/staff00.jpg ./end
mv thdat_output/staff*.png ./end
mv thdat_output/etama*.png ./etama
mv thdat_output/face*.png ./face
mv thdat_output/front*.png ./front
mv thdat_output/loading.png ./image
mv thdat_output/player*.png ./player00
mv thdat_output/music.jpg ./result
mv thdat_output/music*.png ./result
mv thdat_output/result.jpg ./result
mv thdat_output/result*.png ./result
mv thdat_output/stg*bg*.png ./stg1bg
mv thdat_output/stg*enm*.png ./stgenm
mv thdat_output/replay*.png ./title
mv thdat_output/select00.jpg ./title
mv thdat_output/select*.png ./title
mv thdat_output/slpl*.png ./title
mv thdat_output/th06logo.jpg ./title
mv thdat_output/title00.jpg ./title
mv thdat_output/title*.png ./title
mv thdat_output/*.wav ./wav

mv thdat_output/*.pos ../bgm/
mv thdat_output/*.mid ../bgm/

mv thdat_output/*.* ./
rm -rf thdat_output

echo "Finished moving files!"

cd ..

echo "Processing anm files..."

process_anm_folder "${GAME_LOCATION%/}/data"

echo "Removing alpha-only textures..."
find "${GAME_LOCATION%/}/data" -type f -name '*_a.png' -delete

echo "Decompiling ANM files..."
for anm_file in "${GAME_LOCATION%/}/data"/*.anm; do
    tmp_file="${anm_file}.txt"
    printf "Decompiling %s...\n" $anm_file
    truanm d "$anm_file" -g 6 -m ${GAME_LOCATION%/}/th06.anmm > $tmp_file
    rm $anm_file
    mv $tmp_file "${tmp_file%.txt}"
done

echo "Decompiling ECL files..."
for ecl_file in "${GAME_LOCATION%/}/data"/*.ecl; do
    tmp_file="${ecl_file}.txt"
    printf "Decompiling %s...\n" $ecl_file
    truecl d "$ecl_file" -g 6 -m ${GAME_LOCATION%/}/th06.eclm > $tmp_file
    rm $ecl_file
    mv $tmp_file "${tmp_file%.txt}"
done

echo "Converting WAV files to OGG..."
for wav_file in ./wav/*.wav; do
    if [ -f "$wav_file" ]; then
        ogg_file="${wav_file%.wav}.ogg"
        ffmpeg -i "$wav_file" -c:a libvorbis -q:a 4 "$ogg_file" -y -loglevel quiet
        rm "$wav_file"
    fi
done

echo "Done!"
