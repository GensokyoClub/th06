        ...
        if (this->timelineTime.current == this->timelineInstr->time) {
            ...
            case 0:
                if (!g_Gui.BossPresent()) {
                    ...
                    this->SpawnEnemy(this->timelineInstr->arg0,
                                     args1->Var1AsVec(), args1->ushortVar1,
            ...
            case 1:
                if (!g_Gui.BossPresent()) {
                    this->SpawnEnemy(this->timelineInstr->arg0,
                                     this->timelineInstr->args.Var1AsVec(), -1,
                                     ITEM_NO_ITEM, -1);
                }
                break;
            case 2:
                if (!g_Gui.BossPresent()) {
                    ...
                    spawnedEnemy = this->SpawnEnemy(
                        this->timelineInstr->arg0, args2->Var1AsVec(),
                        args2->ushortVar1, args2->ushortVar2, args2->uintVar4);
            ...
            case 3:
                if (!g_Gui.BossPresent()) {
                    spawnedEnemy =
                        this->SpawnEnemy(this->timelineInstr->arg0,
                                         this->timelineInstr->args.Var1AsVec(),
                                         -1, ITEM_NO_ITEM, -1);
                    spawnedEnemy->flags.unk4 = 1;
                }
                break;
...
bool EnemyManager::AddedCallback(EnemyManager *enemyManager) {
    ...
    if (enemyManager->stgEnm2AnmFilename &&
        !g_AnmManager->LoadAnm(ANM_FILE_ENEMY2,
                               enemyManager->stgEnm2AnmFilename,
                               ANM_OFFSET_ENEMY)) {
        return false;
    }
