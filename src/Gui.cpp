...
void Gui::ShowBombNamePortrait(u32 sprite, const char *bombName) {
    ...
    g_AnmManager->DrawVmTextFmt(g_AnmManager, &this->impl->bombSpellcardName,
                                0xf0f0ff, 0x0, bombName);
...
void Gui::ShowSpellcard(i32 spellcardSprite, char *spellcardName) {
    ...
    AnmManager::DrawStringFormat(g_AnmManager, &this->impl->enemySpellcardName,
                                 0xfff0f0, COLOR_RGB(COLOR_BLACK),
                                 spellcardName);
...
bool Gui::ActualAddedCallback() {
    ...
    if ((i32)(g_Supervisor.curState != SUPERVISOR_STATE_GAMEMANAGER_REINIT)) {
        ...
        case CHARA_REIMU:
            if (!g_AnmManager->LoadAnm(ANM_FILE_FACE_CHARA_A,
                                       "data/face00a.anm",
                                       ANM_OFFSET_FACE_CHARA_A)) {
                return false;
            }
            if (!g_AnmManager->LoadAnm(ANM_FILE_FACE_CHARA_B,
                                       "data/face00b.anm",
                                       ANM_OFFSET_FACE_CHARA_B)) {
                return false;
            }
            if (!g_AnmManager->LoadAnm(ANM_FILE_FACE_CHARA_C,
                                       "data/face00c.anm",
                                       ANM_OFFSET_FACE_CHARA_C)) {
                return false;
            }
            break;
        case CHARA_MARISA:
    ...
    AnmManager::DrawStringFormat2(
        g_AnmManager, &this->impl->stageNameSprite, COLOR_RGB(COLOR_LIGHTCYAN),
    ...
    AnmManager::DrawStringFormat(
        g_AnmManager, &this->impl->songNameSprite, COLOR_RGB(COLOR_LIGHTCYAN),
...
bool Gui::LoadMsg(const char *path) {
    ...
    if (this->impl->msg.msgFile == NULL) {
        GameErrorContext::Log(&g_GameErrorContext,
                              TH_ERR_GUI_MSG_FILE_CORRUPTED, path);
        return false;
    }
    this->impl->msg.currentMsgIdx = 0xffffffff;
    this->impl->msg.currentInstr = NULL;

    this->impl->msg.instrs = (MsgRawInstr **)std::malloc(
...
bool GuiImpl::RunMsg() {
    ...
    while ((i32)(this->msg.timer.current >= this->msg.currentInstr->time)) {
        ...
        case MSG_OPCODE_TEXTDIALOGUE:
            ...
            this->msg.dialogueLines[args->text.textLine].fontWidth =
                this->msg.dialogueLines[args->text.textLine].fontHeight =
                    this->msg.fontSize;
            AnmManager::DrawVmTextFmt(
                g_AnmManager, &this->msg.dialogueLines[args->text.textLine],
                this->msg.textColorsA[args->text.textColor],
        ...
        case MSG_OPCODE_TEXTINTRO:
            ...
            AnmManager::DrawStringFormat(
                g_AnmManager, &this->msg.introLines[args->text.textLine],
                this->msg.textColorsA[args->text.textColor],
                this->msg.textColorsB[args->text.textColor], args->text.text);
            this->msg.framesElapsedDuringPause = 0;
            break;
...
bool GuiImpl::DrawDialogue() {
    ...
    //    vertices[0].position.w = vertices[1].position.w =
    //    vertices[2].position.w = vertices[3].position.w = 1.0f;
...
void Gui::DrawGameScene() {
    ...
    if (this->flags.flag2 ||
        ...
        if (g_GameManager.currentPower > 0) {
            //            std::memcpy(&vertices[0].position, &ZunVec3(496.0f,
            //            186.0f, 0.1f), sizeof(ZunVec3));
            //            std::memcpy(&vertices[1].position,
            //            &ZunVec3(g_GameManager.currentPower + 496 + 0.0f,
            //            186.0f, 0.1f),
            //                   sizeof(ZunVec3));
            //            std::memcpy(&vertices[2].position, &ZunVec3(496.0f,
            //            202.0f, 0.1f), sizeof(ZunVec3));
            //            std::memcpy(&vertices[3].position,
            //            &ZunVec3(g_GameManager.currentPower + 496 + 0.0f,
            //            202.0f, 0.1f),
            //                   sizeof(ZunVec3));
            ...
            //            vertices[0].position.w = vertices[1].position.w =
            //            vertices[2].position.w = vertices[3].position.w = 1.0;
            ...
            //            g_Supervisor.d3dDevice->SetVertexShader(D3DFVF_DIFFUSE
            //            | D3DFVF_XYZRHW);
            //            g_Supervisor.d3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP,
            //            2, vertices, sizeof(VertexDiffuseXyzrhw));
            g_AnmManager->SetCurrentBlendMode(0xff);
