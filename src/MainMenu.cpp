...
static const char *const g_ShortCharacterList[4] = {"ReimuA ", "ReimuB ",
                                                    "MarisaA", "MarisaB"};
static const char *const g_DifficultyList[5] = {"Easy   ", "Normal ", "Hard   ",
                                                "Lunatic", "Extra  "};
static const char *const g_StageList[7] = {
    ...
    case STATE_CHARACTER_SELECT:
        ...
        if (WAS_PRESSED_PERIODIC(TH_BUTTON_LEFT)) {
            ...
            if (g_GameManager.difficulty == EXTRA &&
                g_GameManager.HasReachedMaxClears(menu->cursor, 0) == 0 &&
...
#endif
                    replayData =
                        (ReplayHeader *)FileSystem::OpenPath(replayFilePath, 1);
                    if (replayData == NULL) {
                        std::free(replayData);
                        continue;
                    }
            ...
            if (WAS_PRESSED(TH_BUTTON_SELECTMENU)) {
                ...
                this->currentReplay->header =
                    (ReplayHeader *)FileSystem::OpenPath(
                        this->replayFilePaths[this->chosenReplay], 1);
...
ZunResult MainMenu::DrawReplayMenu() const {
    ...
    for (i = this->chosenReplay - this->chosenReplay % 15, replayAmount = i;
        ...
        g_AsciiManager.AddFormatText(
            &vmRef->pos, "%s %8s  %8s %7s  %7s", this->replayFileName[i],
