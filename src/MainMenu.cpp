...
ChainCallbackResult MainMenu::OnUpdate(MainMenu *menu) {
    ...
    case STATE_SHOT_SELECT:
        MoveCursor(menu, 2);
        if (g_GameManager.difficulty == EXTRA && g_GameManager.HasReachedMaxClears(g_GameManager.character, menu->cursor) == 0) {
        ...
        if (WAS_PRESSED(TH_BUTTON_RETURNMENU)) {
            ...
            local_4c =
                g_GameManager.clrd[g_GameManager.CharacterShotType()].difficultyClearedWithoutRetries[g_GameManager.difficulty] > 6
                    ? 6
                    : g_GameManager.clrd[g_GameManager.CharacterShotType()].difficultyClearedWithoutRetries[g_GameManager.difficulty];
    ...
    case STATE_PRACTICE_LVL_SELECT:
        chosenStage = g_GameManager.clrd[g_GameManager.CharacterShotType()].difficultyClearedWithoutRetries[g_GameManager.difficulty] > 6
                          ? 6
                          : g_GameManager.clrd[g_GameManager.CharacterShotType()].difficultyClearedWithoutRetries[g_GameManager.difficulty];
...
ZunResult MainMenu::BeginStartup() {
    ...
    if (g_Supervisor.startupTimeBeforeMenuMusic > 0) {
        time = SDL_GetTicks();
        while ((time - g_Supervisor.startupTimeBeforeMenuMusic >= 0) && (1000 > time - g_Supervisor.startupTimeBeforeMenuMusic)) {
            time = SDL_GetTicks();
        }
...
ZunResult MainMenu::DrawReplayMenu() const {
    ...
    for (i = this->chosenReplay - this->chosenReplay % 15, replayAmount = i; i < replayAmount + 15; i++) {
        ...
        g_AsciiManager.AddFormatText(&vmRef->pos, "%s %8s  %8s %7s  %7s", this->replayFileName[i], this->replayFileData[i].header->name,
    ...
    if (this->stateTimer >= 32) {
        ...
        if (WAS_PRESSED(TH_BUTTON_SELECTMENU)) {
            ...
            case CURSOR_OPTIONS_POS_EXIT:
                ...
                this->cursor = 6;
                g_SoundPlayer.PlaySoundByIdx(SOUND_BACK);
                if (this->colorMode16bit != g_Supervisor.cfg.colorMode16bit || this->windowed != g_Supervisor.cfg.windowed ||
                    this->frameskipConfig != g_Supervisor.cfg.frameskipConfig) {
...
ZunResult MainMenu::ChoosePracticeLevel() const {
    if (this->gameState == STATE_PRACTICE_LVL_SELECT) {
        ...
        i32 selectedStage = (g_GameManager.clrd[charShotType].difficultyClearedWithoutRetries[g_GameManager.difficulty] > 6)
                                ? 6
...
ChainCallbackResult MainMenu::OnDraw(MainMenu *menu) {
    ...
    if (menu->framesActive != 0) {
        ...
        ScreenEffect::DrawSquare(&window,
