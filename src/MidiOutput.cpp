...
void MidiTimer::StartTimer(u32 delay, SDL_TimerCallback cb, void *data) {
    ...
    if (cb != NULL) {
        ...
        this->timerId = SDL_AddTimer(
            delay, (SDL_TimerCallback)&MidiTimer::DefaultTimerCallback, this);
    }
}
...
bool MidiOutput::ReadFileData(u32 idx, const char *path) {
    ...
    if (this->midiFileData[idx] == NULL) {
        g_GameErrorContext.Log(&g_GameErrorContext,
                               TH_ERR_MIDI_FAILED_TO_READ_FILE, path);
        return false;
    }
...
bool MidiOutput::ParseFile(i32 fileIdx) {
    ...
    //  of a sequence 2: the file contains one or more sequentially independent
    //  single-track patterns
    this->format = SDL_SwapBE16(*(u16 *)endOfHeaderPointer);

    // Read the divisions in this track. Note that this doesn't appear to
    // support "negative SMPTE format", which happens when the MSB is set.
...
// Windows EoSD relies solely on the number of times this function is called for
// timing,
//   assuming that there is exactly 1 ms between calls. In my testing, the time
//   between calls with the SDL timer actually ends up averaging to 1.08 ms and
//   the MIDI playback ends up noticeably slow, so the timing mechanism has been
//   replaced with getting a delta from SDL_GetTicks instead.
...
void MidiOutput::ProcessMsg(MidiTrack *track) {
    ...
    case MIDI_OPCODE_MODE_CHANGE:
        ...
        // EoSD doesn't actually use these last two for their intended purpose,
        // instead
        //   using the breath controller to identify the target of a loop within
        //   the file and the foot controller to identify the loop point. Why
        //   did Zun do it like this instead of adding a meta event? Who
        //   knows...
        case 2:
            // Breath control
