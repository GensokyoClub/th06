...
#define TH_BUTTON_REPLAY_CAPTURE (TH_BUTTON_SHOOT | TH_BUTTON_BOMB | TH_BUTTON_FOCUS | TH_BUTTON_SKIP | TH_BUTTON_DIRECTION)
...
ChainCallbackResult ReplayManager::OnDraw(ReplayManager *mgr) { return CHAIN_CALLBACK_RESULT_CONTINUE; }

inline StageReplayData *AllocateStageReplayData(i32 size) { return (StageReplayData *)std::malloc(size); }
    ...
    if (g_ReplayManager != NULL) {
        ...
        if (!mgr->IsDemo()) {
            if (replayPath != NULL) {
                ...
                for (stageIdx = 0; stageIdx < ARRAY_SIZE_SIGNED(g_ReplayManager->replayData->stageReplayData); stageIdx += 1) {
                    if (mgr->replayData->stageReplayData[stageIdx] != NULL) {
                        replayCopy.stageReplayDataOffsets[stageIdx] = (u32)stageReplayPos;
                        stageReplayPos +=
                            (size_t)((u8 *)mgr->replayInputStageBookmarks[stageIdx] - (u8 *)mgr->replayData->stageReplayData[stageIdx]);
                    }
                }
                ...
                checksum = 0x3f000318;
                for (stageIdx = 0; stageIdx < sizeof(ReplayHeader) - offsetof(ReplayHeader, key); stageIdx += 1, checksumCursor += 1) {
                    checksum += *checksumCursor;
                }
                ...
                for (stageIdx = 0; stageIdx < ARRAY_SIZE_SIGNED(mgr->replayData->stageReplayData); stageIdx += 1) {
                    if (mgr->replayData->stageReplayData[stageIdx] != NULL) {
                        std::fwrite(mgr->replayData->stageReplayData[stageIdx], 1,
                                    ((iptr)mgr->replayInputStageBookmarks[stageIdx]) - ((iptr)mgr->replayData->stageReplayData[stageIdx]),
                                    file);
                    }
                }
                std::fclose(file);
