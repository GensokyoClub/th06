...
i32 ResultScreen::HandleResultKeyboard() {
    ...
    if (this->frameTimer == 0) {
        ...
        AnmManager::DrawStringFormat2(
            g_AnmManager, this->unk_28a0, COLOR_RGB(COLOR_WHITE),
            COLOR_RGB(COLOR_BLACK), g_CharacterList[this->charUsed * 2]);
        if (g_GameManager.shotType != SHOT_TYPE_A) {
            this->unk_28a0[0].color = COLOR_TRANSPARENT_WHITE;
        }

        AnmManager::DrawStringFormat2(
            g_AnmManager, &this->unk_28a0[1], COLOR_RGB(COLOR_WHITE),
            COLOR_RGB(COLOR_BLACK), g_CharacterList[this->charUsed * 2]);
...
#endif
                replayLoaded =
                    (ReplayHeader *)FileSystem::OpenPath(replayToReadPath);
                if (replayLoaded == NULL) {
                    continue;
                }

                if (ReplayManager::ValidateReplayData(replayLoaded,
                                                      g_LastFileSize)) {
...
ChainCallbackResult ResultScreen::OnUpdate(ResultScreen *resultScreen) {
    ...
    case RESULT_SCREEN_STATE_BEST_SCORES_LUNATIC:
        ...
        if (resultScreen->charUsed != resultScreen->cursor &&
            ...
            AnmManager::DrawStringFormat2(
                g_AnmManager, &resultScreen->unk_28a0[0],
                COLOR_RGB(COLOR_WHITE), COLOR_RGB(COLOR_BLACK),
                g_CharacterList[resultScreen->charUsed * 2]);
            AnmManager::DrawStringFormat2(
                g_AnmManager, &resultScreen->unk_28a0[1],
                COLOR_RGB(COLOR_WHITE), COLOR_RGB(COLOR_BLACK),
            ...
            for (i = resultScreen->lastSpellcardSelected * 10;
                ...
                if (g_GameManager.catk[i].numAttempts == 0) {
                    AnmManager::DrawVmTextFmt(
                        g_AnmManager, &resultScreen->unk_28a0[i % 10],
                        COLOR_RGB(COLOR_WHITE), COLOR_RGB(COLOR_BLACK),
                        TH_UNKNOWN_SPELLCARD);
                } else {
                    AnmManager::DrawVmTextFmt(
                        g_AnmManager, &resultScreen->unk_28a0[i % 10],
                        COLOR_RGB(COLOR_WHITE), COLOR_RGB(COLOR_BLACK),
                        g_GameManager.catk[i].name);
                }
            }
        }
        if (resultScreen->frameTimer < 30) {
