...
#define COLOR_SET_ALPHA2(color, alpha) \
    (COLOR_RGB(color) | (((alpha) & 0xff) << 24))
#define COLOR_SET_ALPHA3(color, alpha) (COLOR_RGB(color) | ((alpha) << 24))
#define COLOR_COMBINE_ALPHA(color, alpha) \
    (((alpha) & COLOR_ALPHA_MASK) | COLOR_RGB(color))
...
#define COLOR_SET_COMPONENT(color, component, value) \
    ((u8 *)&(color))[(component)] = (value);
