...
//    giving horrid codegen, but on just about every other platform, memcpy gets
//    inlined and produces equal or better codegen than a manual implementation.
...
// endian.
//   Aside from dodging the byteswap, this is also necessary because if GCC
...
template <typename T> struct LE {
    ...
    // Prevent any situation where a struct containing LEs of some type is
    // copied, the compiler assumes
    //   an alignment, and the resulting copy segfaults. Instead we throw a
    //   compile-time error
