...
enum Capabilities { CAPS_BLEND, CAPS_DEPTH_TEST };
...
enum ClearBits { CLEAR_COLOR_BUFFER = 1, CLEAR_DEPTH_BUFFER = 2 };
...
struct GfxInterface {
    ...
    // the Init function can have any arbitrary name since you just have to add
    // it to the s_RenderBackends array in GameWindow.cpp, but it should return
    // a pointer to an instance of your implementation of GfxInterface if it
    // succeeds and call the destructor of that instance and return nullptr if
    // it fails

    virtual ~GfxInterface() = default;
    // the destructor is called when the backend fails to initialize or when the
    // program exits, so it should clean up any resources that were allocated by
    // the init function The GL backends delete the SDL window and GL context,
    // for example
