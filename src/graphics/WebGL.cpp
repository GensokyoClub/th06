...
// Provided for anyone who wants to recompile with new shaders without rerunning
// the premake script
#ifdef USE_C23_EMBED
...
bool WebGL::Init() {
    ...
    this->vertexShaderHandle =
        createShader(vertShaderBytes, GL_VERTEX_SHADER, "vertex");
    this->fragmentShaderHandle =
        createShader(fragShaderBytes, GL_FRAGMENT_SHADER, "fragment");
...
void WebGL::SetTransformMatrix(TransformMatrix type, ZunMatrix &matrix) {
    ...
    // possibilies
    u32 matrixUniformEnum[4] = { UNIFORM_MODELVIEW, UNIFORM_MODELVIEW,
                                 UNIFORM_PROJECTION, UNIFORM_TEXTURE_MATRIX };

    g_glFuncTable.glUniformMatrix4fv(this->uniforms[matrixUniformEnum[type]], 1,
                                     false, (GLfloat *)&matrix.m);
}
